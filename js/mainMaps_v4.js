...
var colorLeft = d3.scaleQuantile()
	.domain([0, 300])
...
function fillLeft(d){
	if (crime_value_dict[String(curYear).concat(d.properties.zip).concat(curCrimeType)] != null) {
		return colorLeft(Math.log(crime_value_dict[String(curYear).concat(d.properties.zip).concat(curCrimeType)]))
	}
	else {
		return colorLeft(0)
	}
